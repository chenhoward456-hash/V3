        ...
        <div className="flex items-center gap-2">
          <div>
            ...
            {healthScore.daysInCycle != null && (
              <p className="text-[11px] text-emerald-500">第 {healthScore.daysInCycle} 天 / 90 天季度</p>
            )}
          </div>
        </div>
        <div className="text-right">
          <div className="flex items-center gap-1.5">
            <span className="text-2xl font-black text-emerald-700 tabular-nums">{healthScore.total}</span>
            <div>
              ...
              <p className="text-[11px] text-emerald-600">健康分數</p>
            </div>
          </div>
        </div>
      </div>
          ...
          return (
            <div key={p.pillar} className="text-center">
              ...
              <p className={`text-[11px] font-medium tabular-nums ${
                p.score >= 80 ? 'text-emerald-600' : p.score >= 50 ? 'text-amber-600' : 'text-rose-500'
              }`}>{p.score}</p>
