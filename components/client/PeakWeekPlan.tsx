  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-5 mb-6">
      ...
      {focusPlan && (
        <div className={`${phaseColors[focusPlan.phase]?.bg || 'bg-gray-50'} ${phaseColors[focusPlan.phase]?.border || 'border-slate-200'} border rounded-2xl p-4 mb-5`}>
          ...
          <div className="grid grid-cols-3 gap-2 mb-3">
            {[
              { label: '碳水', value: focusPlan.carbs, unit: 'g' },
              { label: '蛋白質', value: focusPlan.protein, unit: 'g' },
              { label: '脂肪', value: focusPlan.fat, unit: 'g' },
              { label: '熱量', value: focusPlan.calories, unit: 'kcal' },
              { label: '飲水', value: `${(focusPlan.water / 1000).toFixed(1)}`, unit: 'L' },
              { label: '鈉', value: focusPlan.sodiumMg, unit: 'mg' },
                  ...
                  {focusPlan.showDayTimeline.map((meal, i) => (
                    <div key={i} className="relative flex gap-3">
                      ...
                      <div className={`relative z-10 w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                        ...
                        <span className={`inline-block w-2 h-2 rounded-full ${meal.relativeHours === 0 ? 'bg-white' : meal.relativeHours === -1.5 ? 'bg-rose-400' : 'bg-amber-400'}`} />
                      </div>
            ...
            <div className="space-y-2 text-xs">
              ...
              {focusPlan.foodNote && (
                <div className="bg-white/60 rounded-lg px-3 py-2 border border-gray-200">
                  <p className="font-semibold text-gray-700 mb-0.5">食物來源建議</p>
      ...
      <div className="mb-4 bg-gray-50 border border-gray-200 rounded-xl p-3">
        <p className="text-xs font-semibold text-gray-600 mb-2">每日實際體重（用於 Spillover 偵測）</p>
                ...
                <div className={`ml-4 mr-2 mt-1 mb-2 px-4 py-3 rounded-xl border ${colors.bg} ${colors.border}`}>
                  ...
                  {day.showDayTimeline && day.showDayTimeline.length > 0 ? (
                    <div className="border-t border-gray-200 pt-2 space-y-2">
                      ...
                      {day.showDayTimeline.map((meal, mi) => (
                        <div key={mi} className="flex gap-2 text-[11px]">
                          <span className="inline-block w-1.5 h-1.5 rounded-full bg-amber-400 shrink-0 mt-1" />
                          <div>
                    ...
                    <div className="space-y-1.5 text-[11px] text-gray-600 border-t border-gray-200 pt-2">
                      ...
                      <p><strong>訓練：</strong>{day.trainingNote}</p>
                      ...
                      {day.foodNote && (
                        <div className="mt-1 bg-white/60 rounded-lg px-2.5 py-2 border border-gray-200">
                          <p><strong>食物來源：</strong>{day.foodNote}</p>
                        </div>
                      )}
                  ...
                  {day.expectedWeight && (
                    <p className="text-[11px] text-gray-600 mt-2"><strong>預估體重：</strong>{day.expectedWeight}kg{day.weightNote ? ` — ${day.weightNote}` : ''}</p>
                  )}
                </div>
              )}
            </div>
