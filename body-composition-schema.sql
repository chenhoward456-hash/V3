...
CREATE POLICY "Public can view body_composition via client" ON body_composition FOR SELECT USING (true);
CREATE POLICY "Only authenticated can modify body_composition" ON body_composition FOR ALL USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated'));
