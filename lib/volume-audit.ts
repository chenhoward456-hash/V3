...
export const EXERCISE_MUSCLE_MAP: Record<string, ExerciseEntry> = {
  ...
  // ⚠️ 2026-09-23：跑了全部在籍學員的課表健檢，這批是 unresolved 的殘留。
  //    留著不補的話，那幾個部位的組數會整批偏低，健檢報出來的缺口就不可信。
  '史密斯臥推': { muscle: 'chest', also: ['delts_front', 'triceps'], pattern: 'h_push' },
  '啞鈴側舉': { muscle: 'delts_side', pattern: 'iso', note: '＝側平舉的另一種講法' },
  '繩索臀踢': { muscle: 'glutes', pattern: 'iso', note: 'cable kickback，髖伸單關節' },
  '壺鈴側蹲': { muscle: 'adductors', also: ['quads', 'glutes'], pattern: 'lunge', note: 'cossack squat，側向跨步＋內收肌拉長' },
  '原地彈跳': { muscle: 'calves', pattern: 'iso', volume: false, note: 'pogo jump＝彈性／腳踝剛性的技術練習，不當肌肥大量算' },
  'pogo': { muscle: 'calves', pattern: 'iso', volume: false },
  '貓牛': { muscle: 'core', pattern: 'prep', volume: false, note: '胸椎活動度暖身' },
  '胸椎旋轉': { muscle: 'core', pattern: 'prep', volume: false },
  '世界上最好的伸展': { muscle: 'core', pattern: 'prep', volume: false, note: "world's greatest stretch" },
  '壺鈴繞頸': { muscle: 'delts_side', pattern: 'prep', volume: false, note: 'Halo，肩關節暖身' },
  '風車': { muscle: 'core', also: ['delts_side'], pattern: 'iso', note: 'windmill，側鏈＋過頭穩定' },
  'windmill': { muscle: 'core', also: ['delts_side'], pattern: 'iso' },
  'hyrox': { muscle: 'core', pattern: 'cardio', volume: false, note: '團課／混合體能，不拆成單一部位' },
  'f45': { muscle: 'core', pattern: 'cardio', volume: false },
