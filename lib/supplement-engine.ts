  ...
  if (vitd?.value != null) {
    if (vitd.value < 20) {
      ...
      suggestions.push({
        ...
        dosage: '立即停止 D3 補品',
        timing: '—',
        reason: `維生素 D ${vitd.value} ng/mL，高於一般建議上限（100 ng/mL）。建議先暫停 D3 補充。${vitd.value > 150 ? '數值明顯偏高，建議儘快就醫由醫師評估。' : '減少補充後通常 2-3 個月會自然回降，並建議就醫追蹤。'}`,
        priority: vitd.value > 150 ? 'high' : 'medium',
        evidence: 'Holick 2011 (JCEM, PMID 21646368)：25(OH)D > 100 ng/mL 高於建議範圍，建議由醫師評估',
        triggerTests: [vitd.test_name],
        category: 'deficiency',
      })
    }
  ...
  if (zinc?.value != null) {
    if (zinc.value < 80) {
      ...
      suggestions.push({
        ...
        dosage: '立即停止含鋅補品',
        timing: '—',
        reason: `鋅 ${zinc.value} μg/dL，偏高（參考值 70-120）。長期過量鋅可能影響銅的吸收與免疫功能。建議停止含鋅補品。${zinc.value > 200 ? '建議就醫由醫師評估血清銅。' : '停止補充後 4-6 週建議回檢確認。'}`,
    ...
    if (hemoglobin.value < threshold) {
      ...
      if (alreadyHasIron) {
        ...
        suggestions.push({
          ...
          dosage: '—',
          timing: '—',
          reason: `血紅素 ${hemoglobin.value} g/dL，低於一般參考下限（${gender === '女性' ? '女性 12.0' : '男性 13.5'} g/dL），可能影響有氧代謝與運動表現。成因多樣，建議就醫由醫師評估鐵代謝（ferritin/transferrin）與相關指標，判斷是否需要補鐵或進一步檢查，勿自行服用鐵劑。`,
          priority: 'high',
          evidence: '血紅素低於一般參考下限。建議就醫評估鐵代謝（ferritin/transferrin）由醫師決定後續',
  ...
  if (gender === '男性') {
    ...
    if (e2?.value != null && e2.value > 40) {
      suggestions.push({
        ...
        dosage: '200-300mg',
        timing: '隨餐服用',
        reason: `雌二醇 ${e2.value} pg/mL，高於一般男性參考範圍（10-40）。可留意酒精攝取、增加十字花科蔬菜；部分研究探討 DIM 與雌激素代謝的關聯。如持續偏高建議就醫評估。`,
    ...
    if (isCutting && (serotoninRisk === 'moderate' || serotoninRisk === 'high')) {
      ...
      if (!alreadyHasAshwagandha) {
        suggestions.push({
          ...
          dosage: '600mg',
          timing: '晚餐後或睡前',
          reason: `備賽減脂期皮質醇顯著升高，情緒/壓力調適相關基因型（5-HTTLPR）${isHighRisk ? '較敏感' : '中等敏感'}者更易受影響。南非醉茄研究顯示有助降低皮質醇，備賽高壓期可留意情緒與睡眠。`,
          priority: 'high',
          evidence: 'Chandrasekhar et al. 2012 (Indian J Psychol Med)：KSM-66 降低皮質醇 27.9%',
          triggerTests: [],
          category: 'hormonal',
