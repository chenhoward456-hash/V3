 ...
 *    模糊的規則放前面會把具體的寫法吃掉。
 */
function extractSets(s: string): { sets: number; confidence: 'exact' | 'guess'; rule: string; consumed: string } | null {
  // ① 明寫「N組」——最沒有歧義
  ...
  if (m2) return { sets: +m2[2], confidence: 'exact', rule: '下x組', consumed: m2[0] }

  // ③ 「4x8下」——反過來寫，前面那個是組數
  const m3 = s.match(/(\d+)\s*[x*]\s*(\d+)\s*[下次]/)
  if (m3) return { sets: +m3[1], confidence: 'exact', rule: '組x下', consumed: m3[0] }

  // ④ 三個數 `45x1x10`＝重量x組x次（六角槓硬舉那種寫法）
  //    ⚠️ 真實案例：`六角槓硬舉 45×1×10` 被讀成 45 組過。中間那個才是組數。
  const m4 = s.match(/(\d+(?:\.\d+)?)\s*[x*]\s*(\d+)\s*[x*]\s*(\d+)/)
  if (m4) return { sets: +m4[2], confidence: 'exact', rule: '重量x組x次', consumed: m4[0] }
...
export function parseWorkout(text: string): ParseResult {
  ...
  for (const rawLine of splitLines(text ?? '')) {
    ...
    exercises.push({ raw, name, sets: hit.sets, confidence: hit.confidence, rule: hit.rule })
  }
