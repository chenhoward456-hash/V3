        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          ...
          {filteredClients.length === 0 ? <div className="p-8 text-center"><p className="text-gray-500">{search||statusFilter!=='all'?'沒有符合條件的學員':'還沒有學員資料'}</p>{!search&&statusFilter==='all'&&<Link href="/admin/clients/new" className="inline-block mt-4 text-blue-600 hover:text-blue-800 text-sm">新增第一個學員</Link>}</div> : (
            ...
            <div className="sm:hidden divide-y divide-gray-100">
              {filteredClients.map(client => { const stat = clientStats[client.id]; const act = getActivityLabel(client.id); const ckup = getCheckupLabel(client); return (
                <Link key={client.id} href={`/admin/clients/${client.id}/overview`} className="block px-4 py-4 hover:bg-gray-50 active:bg-gray-100 transition-colors">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <span className="text-base font-semibold text-gray-900">{client.name}</span>
                      {client.competition_enabled && <span className="px-1.5 py-0.5 text-[10px] font-medium rounded-full bg-amber-100 text-amber-700">🏆 備賽</span>}
                      <span className={`px-2 py-0.5 text-[10px] font-medium rounded-full ${client.status==='normal'?'bg-green-100 text-green-700':'bg-red-100 text-red-700'}`}>{client.status==='normal'?'正常':'關注'}</span>
                      {client.competition_enabled && client.competition_date && (() => {
                        ...
                        return d > 0 ? <span className="px-1.5 py-0.5 text-[10px] font-bold rounded-full bg-amber-100 text-amber-700">🏆 {d}天</span> : null
                      })()}
                    </div>
