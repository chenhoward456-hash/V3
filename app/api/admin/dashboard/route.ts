...
export async function GET(request: NextRequest) {
  try {
    ...
    const [
      ...
      supabase
        .from('clients')
        .select('id, unique_code, name, gender, status, is_active, expires_at, subscription_tier, client_mode, competition_enabled, health_mode_enabled, body_composition_enabled, nutrition_enabled, wellness_enabled, training_enabled, supplement_enabled, lab_enabled, ai_chat_enabled, simple_mode, goal_type, diet_start_date, prep_phase, coach_last_viewed_at, coach_weekly_note, created_at, line_user_id')
