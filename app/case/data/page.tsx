...
export default function CaseDataPage() {
  return (
    <div className="bg-white">
      ...
      {/* 轉變前後 — 掉髮/亞健康 → 現在(髮量+上台)，看得見的健康 */}
      <section className="px-5 py-12 max-w-3xl mx-auto">
        <h2 className="text-2xl md:text-3xl font-bold text-slate-900 mb-2">不只數字——看得見的轉變</h2>
        <p className="text-slate-500 mb-6 leading-relaxed">
          頭髮、體態都是身體狀態的鏡子。把整套系統（營養／血檢／補充）調對，看不見的數據好了，看得見的地方也跟著回來。（個人經驗，效果因人而異）
        </p>
        <div className="grid grid-cols-2 gap-3">
          <figure className="m-0">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/before.jpg" alt="調整前：掉髮、亞健康" className="w-full aspect-[3/4] object-cover rounded-2xl border border-slate-200" />
            <figcaption className="text-xs text-slate-400 mt-1.5 text-center">調整前 · 掉髮、亞健康</figcaption>
          </figure>
          <figure className="m-0">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/after.jpg" alt="現在：上台、7.8% 體脂" className="w-full aspect-[3/4] object-cover rounded-2xl border border-slate-200" />
            <figcaption className="text-xs text-slate-400 mt-1.5 text-center">現在 · 上台、7.8% 體脂</figcaption>
          </figure>
        </div>
      </section>
